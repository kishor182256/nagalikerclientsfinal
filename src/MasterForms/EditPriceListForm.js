...
const EditPriceListForm = () => {
  ...
  const fetchSubCategory = async () => {
    try {
      ...
      setSubcategory(data?.data?.testSubCategory)
    } catch (e) {
      console.log(e);
    }
  };
  ...
  const handleSubmit = async (values) => {
    try {
      const data = await axios.put(`${API}/edittestcategoryprice/${id}`, values, {
        headers: { authtoken: `${TOKEN}` },
      });
      if(data){
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      collect: subcategory?.collect,
      pricelist: subcategory?.pricelist,
    },
    validationSchema,
        ...
        <div className={classes.collectorForm}>
          <div className={classes.formheader}>
            <div className={classes.formname}>
              <div className={classes.formh2}>Edit Price list </div>
              <div className={classes.formspecification}>
                You can Edit a new report group here
              </div>
            </div>
            <div>
              <Buttons
